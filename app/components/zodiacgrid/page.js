...
const ZodiacGrid = () => {
    ...
    return (<div className="bg-[#ffff] py-2">

            <div className="main-container py-8 px-0 ">
                {/* Heading */}
                <div className="text-center mb-12">
                    <h2 className="text-center text-primaryColor text-3xl font-extrabold mb-5">
                ...
                <div className="grid md:grid-cols-3 gap-6  mx-auto mb-16">
                    {services.map((item, i) => (<div key={i} className="bg-white shadow__md__lists  border-l-4 border-primaryColor p-6 rounded-xl hover:shadow-lg transition">
                ...
                <div className="text-center mb-10">
                    <h3 className="text-center text-primaryColor text-3xl font-extrabold mb-2">
                        The Four Elements in Astrology
                    </h3>
                </div>

                <div className="grid md:grid-cols-4 gap-6  mx-auto">
                    {elements.map((el, i) => (<div key={i} className={`p-6 rounded-xl border shadow-sm hover:shadow-md transition ${el.bg}`}>
            ...
            <div className="main-container mx-auto px-0">
                <div className=" grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
                    {Horoscopeimages.map((item) => (<div key={item.name} className="bg-[#F973160D] hover:bg-orange-200 rounded-xl text-center p-5 cursor-pointer transition duration-300 border-2  shadow-md hover:shadow-lg" onClick={() => handleZodiacClick(item.name)}>
                            <img src={item.img} alt={item.name} className="w-20 h-20 mx-auto mb-4 object-contain rounded-full border-4 border-orange-400" width={80} height={80}/>
            ...
            <div className="py-8">
                <div className="max-w-4xl mx-auto">
                    <div className="bg-white shadow-md border border-l-4 border-orange-500 p-6 rounded-xl hover:shadow-lg transition">
                        ...
                        {contentData.map((item, index) => (<p key={index} className="text-gray-700 mb-4">
                                {item.text}
                            </p>))}
                    </div>
                    ...
                    <div className="mt-14 text-center">
                        <button onClick={() => router.push('/daily-horoscope')} className="bg-[#F973160D]    hover:shadow-lg text-orange-600 px-8 py-3 rounded-full font-semibold text-lg shadow-md hover:bg-orange-100 transition">
