...
import { FaPlay, FaSun, FaMoon, FaRegCalendarAlt, FaStar, FaVideo, } from "react-icons/fa";
...
export default function AstrocallHomepage() {
  ...
  const bestVideo = HomeVideoData?.[0];

  const bestVideoYoutubeId = useMemo(() => {
    if (!bestVideo) return "";
    if (bestVideo.VideoID) return bestVideo.VideoID;
    const match = bestVideo.YoutubeUrl?.match(
  ...
  useEffect(() => {
    const element = bestVideoRef.current;
    if (!element) return;

    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          setIsBestVideoInView(true);
          observer.disconnect();
        }
      },
      { threshold: 0.2 }
  ...
  const handleSelect = (selectedPlace) => {
    setValue(prev => ({ ...prev, BirthPlace: selectedPlace }));
    setLocationdata([]);
    ...
    >
      <div className="max-w-6xl mx-auto px-4 py-12 grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <div className=" bg-white sellerCard  rounded-2xl shadow-lg p-6 transition duration-300 hover:shadow-xl">
          ...
          <ul className="text-sm space-y-3 p-4 text-gray-700">
            <li>
              ...
              Tithi: <span className="font-semibold">{FindTithiData?.tithis?.[0]?.tithi}</span>
            </li>

            <li>
              <FaStar className="inline mr-1" />
              Nakshatra: <span className="font-semibold">{nakshatraData?.nakshatras?.nakshatra_list?.[0]}</span>
            </li>

            <li>
              <FaSun className="inline mr-1" />
              Sunrise: <span className="font-semibold">{sunmoonData?.sunrise}</span>
            </li>

            <li>
              <FaMoon className="inline mr-1" />
              Sunset: <span className="font-semibold">{sunmoonData?.sunset}</span>
            </li>
            <li>
              <FaMoon className="inline mr-1" />
              Moonrise: <span className="font-semibold">{sunmoonData?.moonrise}</span>
            </li>
            <li>
              <FaStar className="inline mr-1" />
              Moonset: <span className="font-semibold">{sunmoonData?.moonset}</span>
            </li>
        ...
        <div className=" bg-white sellerCard  rounded-2xl shadow-lg p-6 transition duration-300 hover:shadow-xl">
          ...
          <div ref={bestVideoRef} className="w-full h-52 overflow-hidden rounded-lg bg-black/5 relative">
            {isBestVideoInView && bestVideoYoutubeId && videoEmbedPlaying ? (
              <iframe
                className="absolute inset-0 h-full w-full"
                src={`https://www.youtube.com/embed/${bestVideoYoutubeId}?autoplay=1&mute=1&loop=1&playlist=${bestVideoYoutubeId}&rel=0`}
                title={HomeVideoData[0]?.Description || "How Astrology Works"}
                allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
                allowFullScreen
                loading="lazy"
              />
            ) : isBestVideoInView && bestVideoYoutubeId ? (
              <button
                type="button"
              ...
              >
                <img
                  src={`https://i.ytimg.com/vi/${bestVideoYoutubeId}/hqdefault.jpg`}
                  alt=""
                  width={480}
                  height={360}
