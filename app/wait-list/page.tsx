...
import { FaClock, FaSearch, FaBell, FaUser, FaStar, FaTimes } from "react-icons/fa";
...
export default function WaitList() {
  ...
  const handleRemoveFromWaitList = (itemId: any) => {
    setWaitList(prev => prev.filter(item => item.id !== itemId));
  };

  const filteredWaitList = waitList.filter(item => 
    item.astrologer.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <>
      <SEO 
        title="Wait List - AstroCall"
        description="Manage your waitlist for astrologer consultations"
      />
      <Header />
                  ...
                  <div className="p-4">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex flex-col gap-2">
                        {item.status === "ready" && (
                          <button
                            onClick={() => router.push(`/talk-to-astrologers/${item.astrologer.id}`)}
