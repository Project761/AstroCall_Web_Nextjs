...
import { TokenWithDeleteUpadateAdd, getPostData } from "../../../utils/api.js";
import { useMenuContext } from "../../../hooks/useMenuContext";
import Header from "../../../components/Header/page.js";
import SEO from "../../../components/SEO/page.js";
const OnlinepujaPlansDetails = () => {
    const { loginUserData, setLoginUserData, pujareviewstatus, setpujareviewstatus, Get_Data_RazorPayKey, RazorPayKey } = useMenuContext();
    const { slug } = useParams();
    const UserLoginId = typeof window !== 'undefined' ? localStorage.getItem("UserLoginId") || "" : "";
    const router = useRouter();
    const [plansdata, setplansdata] = useState([]);
    const [onlinepujaID, setonlinepujaID] = useState(null);
    const [loading, setLoading] = useState(false);
    useEffect(() => {
        if (UserLoginId && Get_Data_RazorPayKey) {
            Get_Data_RazorPayKey();
        }
    }, [UserLoginId, Get_Data_RazorPayKey]);
    useEffect(() => {
        if (slug) {
            GetsinglaData_Puja();
        }
    }, [slug]);
    const TotalAmt = plansdata.length > 0 ? plansdata[0].TotalAmt : 0;
    const TotalGstAmt = plansdata.length > 0 ? plansdata[0].GSTAmt : 0;
    useEffect(() => {
            ...
            if (response?.OrderId) {
                const options = {
                    ...
                    notes: {
                        ...
                        BonusAmt: "0",
                        UserId: UserLoginId,
                        PaymentType: "OnlinePuja",
                        ProductName: slug,
                        "ProductId": onlinepujaID
                    },
                    handler: function (paymentResponse) {
                        if (paymentResponse?.razorpay_order_id) {
                            Get_Booking_Puja(response.OrderId);
                        }
                    },
                    prefill: {
    ...
    const GetsinglaData_Puja = async () => {
        const val = {
            'PujaID': '0',
            'PujaName': (typeof slug === 'string' ? slug : slug?.[0])?.toLowerCase().replace(/-/g, " ").replace(/\s+/g, " ").trim(),
        };
        try {
            const urlSet = typeof window !== 'undefined' ? window.location.origin : '';
            const res = await axios.post(urlSet === 'https://astrocall.live'
                ? `https://api.astrocall.live/api/Puja/GetsinglaData_Puja`
                : `https://liveapi.astrocall.live/api/Puja/GetsinglaData_Puja`, val);
            const { data } = res;
            const parseData = JSON.parse(data?.data);
            const Resdata = parseData?.Table;
            if (Resdata) {
                setplansdata(Resdata);
                setonlinepujaID(Resdata[0].OnlinePujaID);
                setSelectedPuja(Resdata);
            }
        }
        catch (error) {
            if (error?.response?.status == 400 && error?.response?.data?.Message === "No Data Available") {
                ...
                if (res) {
                    ...
                    setplansdata(filteredPuja);
                    setonlinepujaID(filteredPuja[0].OnlinePujaID);
                }
            }
        }
    };
    const Get_Booking_Puja = async (OrderId) => {
        const val = {
            PujaID: onlinepujaID,
            UserID: UserLoginId,
            OrderStatus: "",
        ...
        try {
            const res = await TokenWithDeleteUpadateAdd("BookingPuja/Insert_BookingPuja", val);
            if (res) {
    ...
    const PaymentMethodBtn = ({ name, icon, onClick }) => {
        const renderIcon = () => {
            if (name === "PhonePe") {
                return (<svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg" fillRule="evenodd" clipRule="evenodd" strokeLinejoin="round" strokeMiterlimit="2">
            ...
            <path d="M372.164 189.203c0-10.008-8.576-18.593-18.584-18.593h-34.323l-78.638-90.084c-7.154-8.577-18.592-11.439-30.03-8.577l-27.17 8.577c-4.292 1.43-5.723 7.154-2.862 10.007l85.8 81.508H136.236c-4.293 0-7.154 2.861-7.154 7.154v14.292c0 10.016 8.585 18.592 18.592 18.592h20.015v68.639c0 51.476 27.17 81.499 72.931 81.499 14.292 0 25.739-1.431 40.03-7.146v45.753c0 12.87 10.016 22.886 22.885 22.886h20.015c4.293 0 8.577-4.293 8.577-8.586V210.648h32.893c4.292 0 7.145-2.861 7.145-7.145v-14.3zM280.65 312.17c-8.576 4.292-20.015 5.723-28.591 5.723-22.886 0-34.324-11.438-34.324-37.176v-68.639h62.915v100.092z" fill="#fff" fillRule="nonzero"/>
                ...
                return (<svg xmlns="http://www.w3.org/2000/svg" shapeRendering="geometricPrecision" textRendering="geometricPrecision" imageRendering="optimizeQuality" fillRule="evenodd" clipRule="evenodd" viewBox="0 0 512 385.414">
            <path fill="#3B95D9" fillRule="nonzero" d="M26.217 0h382.258c14.366 0 26.16 11.803 26.16 26.158V264.76c0 14.364-11.796 26.16-26.16 26.16H26.217c-14.384 0-26.16-11.776-26.16-26.16V26.158C.057 11.798 11.859 0 26.217 0z"/>
            ...
            <path fill="#DACD71" d="M139.602 153.639h56.914c7.258 0 13.197 5.939 13.197 13.197v2.883h-83.307v-2.883c0-7.258 5.938-13.197 13.196-13.197zm70.111 20.621v28.134h-25.844V174.26h25.844zm-30.384 28.134h-22.568V174.26h22.568v28.134zm-27.109 0h-25.814V174.26h25.814v28.134zm57.493 4.541v2.928c0 7.257-5.94 13.196-13.197 13.196h-56.914c-7.257 0-13.196-5.938-13.196-13.196v-2.928h83.307z"/>
                ...
                return (<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 117.34 122.88" xmlSpace="preserve">
            <g>
              <path fillRule="evenodd" clipRule="evenodd" d="M85.14,14.83L43.26,40.28h11.91l30.92-18.79l4.54-2.76l13.09,21.55h7.62 c1.66,0,3.16,0.68,4.25,1.76l0,0c1.09,1.09,1.77,2.59,1.77,4.24v70.59c0,1.65-0.68,3.15-1.76,4.23v0.01 c-1.09,1.09-2.59,1.76-4.25,1.76l-105.33,0c-1.66,0-3.16-0.67-4.25-1.76v-0.01C0.68,120.02,0,118.52,0,116.88V46.28 c0-1.65,0.68-3.16,1.76-4.24c1.09-1.09,2.59-1.76,4.25-1.76h2.5L73.53,0.77v0C74.36,0.27,75.3,0,76.27,0 c0.42,0,0.84,0.05,1.25,0.15c1.36,0.33,2.54,1.19,3.26,2.39v0l6.63,10.91L85.14,14.83z M5.89,45.62 c-0.23,0.25-0.42,0.53-0.56,0.84v8.69h106.68v-8.87c0-0.19-0.07-0.36-0.19-0.47h-0.01c-0.12-0.12-0.29-0.2-0.48-0.2H6.01 C5.97,45.61,5.93,45.61,5.89,45.62z M15.98,84.71h19.05v7.13H15.98V84.71z M15.98,101.59h53.25v6.43H15.98V101.59z M86.21,84.71h19.05v7.13H86.21V84.71z M62.8,84.71h19.05v7.13H62.8V84.71z M39.39,84.71h19.05v7.13H39.39V84.71z M112.01,75.14H5.33v41.73c0,0.19,0.07,0.36,0.2,0.48l0.01,0c0.13,0.13,0.3,0.2,0.47,0.2l105.33,0c0.18,0,0.35-0.08,0.48-0.2 l0,0c0.13-0.13,0.2-0.3,0.2-0.48V75.14z"/>
                ...
                return (<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 122.88 38.52" xmlSpace="preserve">
            ...
            <g>
              ...
              <path className="st0" d="M65.69,6.2h-5.48C59.55,6.2,59,6.74,59,7.41v11.33c-0.01,0.7-0.58,1.26-1.28,1.26h-2.29 c-0.71,0-1.29-0.57-1.29-1.28L54.12,7.41c0-0.67-0.54-1.21-1.21-1.21h-5.48c-0.67,0-1.21,0.54-1.21,1.21v12.41 c0,4.71,3.36,8.08,8.08,8.08c0,0,3.54,0,3.65,0.02c0.64,0.07,1.13,0.61,1.13,1.27c0,0.65-0.48,1.19-1.12,1.27 c-0.03,0-0.06,0.01-0.09,0.02l-8.01,0.03c-0.67,0-1.21,0.54-1.21,1.21v5.47c0,0.67,0.54,1.21,1.21,1.21h8.95 c4.72,0,8.08-3.36,8.08-8.07V7.41C66.9,6.74,66.36,6.2,65.69,6.2L65.69,6.2z M34.53,6.23h-7.6c-0.67,0-1.22,0.51-1.22,1.13v2.13 c0,0.01,0,0.03,0,0.04c0,0.02,0,0.03,0,0.05v2.92c0,0.66,0.58,1.21,1.29,1.21h7.24c0.57,0.09,1.02,0.51,1.09,1.16v0.71 c-0.06,0.62-0.51,1.07-1.06,1.12h-3.58c-4.77,0-8.16,3.17-8.16,7.61v6.37c0,4.42,2.92,7.56,7.65,7.56h9.93 c1.78,0,3.23-1.35,3.23-3.01V14.45C43.34,9.41,40.74,6.23,34.53,6.23L34.53,6.23z M35.4,29.09v0.86c0,0.07-0.01,0.14-0.02,0.2 c-0.01,0.06-0.03,0.12-0.05,0.18c-0.17,0.48-0.65,0.83-1.22,0.83h-2.28c-0.71,0-1.29-0.54-1.29-1.21v-1.03c0-0.01,0-0.03,0-0.04 l0-2.75v-0.86l0-0.01c0-0.66,0.58-1.2,1.29-1.2h2.28c0.71,0,1.29,0.54,1.29,1.21V29.09L35.4,29.09z M13.16,6.19H1.19 C0.53,6.19,0,6.73,0,7.38v5.37c0,0.01,0,0.02,0,0.03c0,0.03,0,0.05,0,0.07v24.29c0,0.66,0.49,1.2,1.11,1.21h5.58 c0.67,0,1.21-0.54,1.21-1.21l0.02-8.32h5.24c4.38,0,7.44-3.04,7.44-7.45v-7.72C20.6,9.25,17.54,6.19,13.16,6.19L13.16,6.19z M12.68,16.23v3.38c0,0.71-0.57,1.29-1.28,1.29l-3.47,0v-6.77h3.47c0.71,0,1.28,0.57,1.28,1.28V16.23L12.68,16.23z"/>
                ...
                return (<svg viewBox="0 0 123.84 104.9" xmlns="http://www.w3.org/2000/svg">
        ...
        return (<button onClick={onClick} className="border border-gray-300 p-2 sm:p-3 rounded-lg hover:bg-gray-50 hover:border-orange-400 flex flex-col items-center justify-center transition gap-1 sm:gap-2 w-full h-20 sm:h-24 shadow-sm hover:shadow-md">
        <div className="w-10 h-10 sm:w-12 sm:h-14 flex items-center justify-center">
          {renderIcon()}
        </div>
        <span className="text-[9px] sm:text-[10px] font-medium text-center leading-tight">{name}</span>
      </button>);
    };
    return (<>
      <SEO title="Online Puja Plans Details" description="Online puja plans details." canonical="https://astrocall.live/online-puja" type="website" schema={{
      ...
      <div className="bg-[#F973160D] pt-20 lg:pt-24">
          ...
          <div className="rounded-md sm:rounded-lg w-full text-white text-center py-6 sm:py-8 md:py-10 px-3 sm:px-4">
            <div className="flex flex-col items-center justify-center">
              <div className="flex items-center gap-2 sm:gap-3 flex-wrap justify-center">
                <MdOutlineTempleBuddhist className="text-white text-2xl sm:text-3xl"/>
                <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold px-2 text-white drop-shadow-lg">Online Puja Plans Details</h1>
              </div>
              <p className="mt-2 sm:mt-3 max-w-xl text-xs sm:text-sm md:text-base leading-relaxed px-2">
                Book customized online puja services for various occasions with certified priests, rituals, and live streaming.
              </p>
        ...
        <div className="heading main-container px-3 sm:px-4 py-6 sm:py-8">
          <h1 className="text-center text-2xl sm:text-3xl md:text-4xl font-[700]">Payment Details</h1>
          <div className="w-[100px] sm:w-[120px] md:w-[150px] h-[2px] sm:h-[3px] m-auto rounded-full bg-primaryColor my-2 sm:my-3"></div>
        </div>

        <div className="main-container px-3 sm:px-4 md:px-6 lg:px-20">
          <div className="flex flex-col lg:flex-row gap-4 sm:gap-6 md:gap-8 py-4 sm:py-6 md:py-8">
            {/* Left Side: Payment Details */}
            <div className="lg:w-1/2 w-full bg-orange-50 p-4 sm:p-6 md:p-8 lg:p-10 rounded-xl sm:rounded-2xl shadow-md border">
              <h2 className="text-lg sm:text-xl font-bold mb-3 sm:mb-4 text-primaryColor">Payment Summary</h2>
              {plansdata?.map((item, index) => (<div key={index} className="space-y-3 sm:space-y-4 text-xs sm:text-sm text-gray-700">
                  <div className="flex flex-col sm:flex-row border-b pb-2 gap-1 sm:gap-0">
                    <span className="font-medium w-full sm:w-1/3">Puja Name:</span>
                    <span className="w-full sm:w-2/3 text-left sm:text-right break-words">
                      {item?.PujaName}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between border-b pb-2">
                    ...
                    <span>₹ {item?.GSTAmt || 0}</span>
                  </div>
                  <div className="flex justify-between font-semibold text-base sm:text-lg pt-3 sm:pt-4 border-t">
                    <span>Total Payable:</span>
                    <span className="text-orange-600">₹ {item?.TotalAmt || 0}</span>
                  </div>
                </div>))}
            </div>

            {/* Right Side: Choose Payment Method */}
            <div className="lg:w-1/2 w-full bg-white p-4 sm:p-6 rounded-xl sm:rounded-2xl shadow-md border">
              <h2 className="text-lg sm:text-xl font-bold mb-4 sm:mb-6 text-primaryColor text-center">
                Choose Payment Method
              </h2>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 text-xs sm:text-sm">
                <PaymentMethodBtn name="Card" icon="/icons/card.svg" onClick={() => handlePayment()}/>
                <PaymentMethodBtn name="Wallets" icon="/icons/wallet.svg" onClick={() => handlePayment()}/>
                <PaymentMethodBtn name="Paytm" icon="/icons/paytm.svg" onClick={() => handlePayment()}/>
                <PaymentMethodBtn name="GPay" icon="/icons/gpay.svg" onClick={() => handlePayment()}/>
                <PaymentMethodBtn name="BHIM UPI" icon="/icons/upi.svg" onClick={() => handlePayment()}/>
              </div>
            </div>
          </div>
        </div>
...
export default OnlinepujaPlansDetails;
