...
import { MdLocalOffer, MdArrowBack } from "react-icons/md";
import { FaBolt, FaShieldAlt, FaLock, FaCheck } from "react-icons/fa";
...
const PaymentMethodBtn = ({ name, icon, onClick }) => {
    ...
    return (
        <button
            type="button"
            onClick={onClick}
            className="group flex flex-col items-center justify-center gap-2 rounded-2xl border border-orange-100 bg-white p-3 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-orange-200 hover:shadow-md active:scale-95"
        >
            <div className="flex h-12 w-12 items-center justify-center">
                {renderIcon()}
            </div>
            <span className="text-center text-[11px] font-semibold leading-tight text-gray-600 transition-colors group-hover:text-[#FF5C00]">
                {name}
            </span>
        </button>
    );
...
const PlansDetailsContent = () => {
    ...
    const { loginUserData, Get_SingleData_User, PlanSuccessPopup, setPlanSuccessPopup, RazorPayKey, Get_Data_RazorPayKey } = useMenuContext();
    ...
    const Get_Data_WalletPackage = useCallback(async (packageId) => {
    ...
    }, [GetWalletPackageID, Get_Data_WalletPackage, Get_Data_RazorPayKey]);
    ...
    const handleApplyCoupon = async () => {
        ...
        setCouponError("");
        try {
            const val = { "OfferCode": couponCode.trim(), "MinRechargeAmount": plansdata[0]?.PackageAmt };
            const res = await postWithToken("UserOffers/GetData_OfferMaster", val);
            if (res) {
                const FilterData = res[0]
                setAppliedCoupon({
                    code: couponCode.trim().toUpperCase(),
                    discountAmt: FilterData.BonusAmount,
                    message: FilterData.Message || `₹${FilterData.BonusAmount} Coupon Bonus Amount`,
                });
                // const UpdateAMT = Number(totalamt) + Number(FilterData.BonusAmount);
                // settotalamt(UpdateAMT)

            } else {
                setCouponError(res?.Message || "Invalid coupon code. Please try again.");
            }
        } catch (error) {
    ...
    const payAmount = Number(plan?.PackageAmt || 0);
    const bonus = Number(plan?.BonusAmt || 0);
    const walletCredit = payAmount + bonus;

    return (
        <div className="min-h-screen pb-16 pt-14 sm:pt-16" style={{ backgroundColor: CREAM }}>
            <section className="border-b border-orange-100 bg-white">
                <div className="main-container px-4 py-5 sm:py-6">
                    <nav className="mb-3 text-xs text-gray-500 sm:text-sm">
                        <Link href="/" className="hover:text-[#FF5C00]">Home</Link>
                        <span className="mx-2">›</span>
                        <Link href="/plans" className="hover:text-[#FF5C00]">Packages</Link>
                        <span className="mx-2">›</span>
                        <span style={{ color: ORANGE }}>Recharge{payAmount ? ` ₹${payAmount}` : ""}</span>
                    </nav>
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-4">
                        <div>
                            <Link
                                href="/plans"
                                className="mb-3 inline-flex items-center gap-1.5 text-xs font-semibold text-gray-500 transition hover:text-[#FF5C00] sm:text-sm"
                            >
                                <MdArrowBack /> Back to Packages
                            </Link>
                            <h1 className="font-serif text-2xl font-extrabold text-[#1A1A1A] sm:text-3xl">
                                Complete Your <span style={{ color: ORANGE }}>Recharge</span>
                            </h1>
                            <p className="mt-2 max-w-xl text-sm text-gray-600">
                                Review your order, apply a coupon if you have one, and pay securely.
                            </p>
                        </div>

                        {payAmount > 0 && (
                            <div
                                className="rounded-2xl border border-orange-100 px-4 py-3 text-center shadow-sm sm:min-w-[180px]"
                                style={{ background: `linear-gradient(135deg, ${PEACH} 0%, white 100%)` }}
                            >
                                <p className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Selected Plan</p>
                                <p className="text-2xl font-extrabold text-[#1A1A1A]">₹{payAmount}</p>
                                {walletCredit > payAmount && (
                                    <p className="mt-0.5 text-xs font-semibold text-green-600">
                                        Get ₹{walletCredit} credit
                                    </p>
                                )}
                            </div>
                        )}
            ...
            <div className="main-container grid grid-cols-1 gap-6 px-4 py-6 sm:py-8 lg:grid-cols-5 lg:gap-8">

                {/* Left Column — Summary + Coupon */}
                <div className="flex flex-col gap-5 lg:col-span-2">

                    <div className="overflow-hidden rounded-2xl border border-orange-100 bg-white shadow-[0_4px_20px_rgba(255,92,0,0.08)]">
                        <div
                            className="flex items-center gap-2 border-b border-orange-100 px-5 py-4"
                        ...
                        >
                            ...
                            <h2 className="text-sm font-bold text-[#1A1A1A]">Order Summary</h2>
                        </div>
                        ...
                        <div className="px-5 py-4">
                            {plansdata?.length > 0 ? (
                                <>
                                    {plansdata?.map((item, index) => (
                                        <div key={index} className="space-y-3 text-sm">
                                            <div className="flex items-center justify-between border-b border-dashed border-orange-50 py-2.5">
                                                <span className="text-gray-500">Recharge Amount</span>
                                                <span className="font-semibold text-[#1A1A1A]">₹{item?.PackageAmt}</span>
                                            </div>
                                            {bonus > 0 && (
                                                <div className="flex items-center justify-between border-b border-dashed border-orange-50 py-2.5">
                                                    <span className="text-green-600">Wallet Bonus</span>
                                                    <span className="font-semibold text-green-600">+₹{item?.BonusAmt}</span>
                                                </div>
                                            )}
                                            <div className="flex items-center justify-between border-b border-dashed border-orange-50 py-2.5">
                                                <span className="text-gray-500">GST <span className="text-xs text-gray-400">(18%)</span></span>
                                                <span className="font-semibold text-[#1A1A1A]">₹{item?.GSTAmt}</span>
                                            </div>
                                            {appliedCoupon && (
                                                <div className="flex items-center justify-between border-b border-dashed border-orange-50 py-2.5">
                                                    ...
                                                    <span className="font-semibold text-green-600">+₹{appliedCoupon.discountAmt}</span>
                                                </div>
                                            )}
                                            <div className="flex items-center justify-between pt-3">
                                                <span className="font-bold text-[#1A1A1A]">Total Payable</span>
                                                <span className="text-xl font-extrabold" style={{ color: ORANGE }}>₹{totalamt}</span>
                                            </div>
                                            {walletCredit > 0 && (
                                                <div className="rounded-xl px-3 py-2.5" style={{ backgroundColor: CREAM_ALT }}>
                                                    <p className="text-[10px] font-semibold text-gray-500">Total Wallet Credit</p>
                                                    <p className="text-lg font-extrabold" style={{ color: ORANGE }}>₹{walletCredit}</p>
                                                </div>
                                            )}
                                        </div>
                                    ))}

                                    {plansdata?.[0]?.BonusAmt > 0 && (
                                        <div className="mt-4 flex items-center gap-2.5 rounded-xl border border-green-200 bg-green-50 px-4 py-3">
                                            <span className="text-xl">🎁</span>
                                            <p className="text-xs font-medium leading-snug text-green-700">
                                                ₹{plansdata?.[0]?.BonusAmt} bonus will be added to your wallet after recharge
                                            </p>
                                        </div>
                                    )}
                                </>
                                ...
                                <div className="flex flex-col items-center gap-3 py-8">
                                    <div className="h-8 w-8 animate-spin rounded-full border-2 border-orange-200 border-t-[#FF5C00]" />
                                    <p className="text-xs text-gray-400">Fetching plan details…</p>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="overflow-hidden rounded-2xl border border-orange-100 bg-white shadow-[0_4px_20px_rgba(255,92,0,0.08)]">
                        ...
                        >
                            ...
                            <h2 className="text-sm font-bold text-[#1A1A1A]">Apply Coupon</h2>
                        </div>
                        ...
                        <div className="px-5 py-4">
                            <div className="flex gap-2">
                                <input
                                    ...
                                    className={`flex-1 rounded-xl border px-4 py-2.5 text-sm font-medium uppercase tracking-widest outline-none transition-all focus:ring-2 focus:ring-orange-200 ${
                                        appliedCoupon
                                ...
                                {!appliedCoupon ? (
                                    <button
                                        ...
                                        className="flex min-w-[80px] items-center justify-center rounded-xl px-5 py-2.5 text-sm font-semibold text-white transition-all active:scale-95 disabled:cursor-not-allowed disabled:opacity-50"
                                        style={{ backgroundColor: ORANGE }}
                                    >
                                        {couponLoading ? (
                                    ...
                                    <button
                                        ...
                                        className="min-w-[80px] rounded-xl border border-red-200 bg-red-50 px-4 py-2.5 text-sm font-semibold text-red-500 transition hover:bg-red-100"
                                    >
                                        Remove
                                    </button>
                                )}
                            ...
                            {couponError && (
                                <div className="mt-2.5 flex items-center gap-1.5 rounded-lg border border-red-100 bg-red-50 px-3 py-2 text-xs text-red-500">
                                    {couponError}
                                </div>
                            )}

                            {appliedCoupon && (
                                <div className="mt-2.5 flex items-center gap-2.5 rounded-xl border border-green-200 bg-green-50 px-4 py-3">
                                    <span className="text-lg">🎉</span>
                                    <div className="min-w-0 flex-1">
                                        <p className="text-xs font-semibold text-green-700">Coupon Applied!</p>
                                        <p className="truncate text-xs text-gray-500">{appliedCoupon.message}</p>
                                    </div>
                                    <span className="whitespace-nowrap text-sm font-bold text-green-600">₹{appliedCoupon.discountAmt}</span>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-3 text-center">
                        {[
                            { icon: FaLock, label: "100% Secure" },
                            { icon: FaBolt, label: "Instant Credit" },
                            { icon: FaShieldAlt, label: "Encrypted" },
                        ].map(({ icon: Icon, label }) => (
                            <div key={label} className="flex flex-col items-center gap-1.5 rounded-xl border border-orange-100 bg-white py-3 px-2 shadow-sm">
                                <Icon className="text-base" style={{ color: ORANGE }} />
                                <span className="text-[10px] font-semibold leading-tight text-gray-600 sm:text-xs">{label}</span>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="lg:col-span-3">
                    <div className="overflow-hidden rounded-2xl border border-orange-100 bg-white shadow-[0_4px_20px_rgba(255,92,0,0.08)]">
                        <div
                            className="flex items-center gap-2 border-b border-orange-100 px-6 py-4"
                            style={{ background: `linear-gradient(90deg, ${CREAM_ALT}, ${CREAM})` }}
                        >
                            <div
                                className="flex h-8 w-8 items-center justify-center rounded-lg text-white"
                            ...
                            <div>
                                <h2 className="text-sm font-bold text-[#1A1A1A]">Choose Payment Method</h2>
                                <p className="mt-0.5 text-[11px] text-gray-500">All transactions are SSL encrypted</p>
                            </div>
                        </div>

                        <div className="px-6 py-5">
                            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                                <PaymentMethodBtn name="Card" icon="/icons/card.svg" onClick={() => handlePayment("card")} />
                                <PaymentMethodBtn name="Wallets" icon="/icons/wallet.svg" onClick={() => handlePayment("wallet")} />
                                <PaymentMethodBtn name="Paytm" icon="/icons/paytm.svg" onClick={() => handlePayment("paytm")} />
                                <PaymentMethodBtn name="GPay" icon="/icons/gpay.svg" onClick={() => handlePayment("gpay")} />
                                <PaymentMethodBtn name="BHIM UPI" icon="/icons/upi.svg" onClick={() => handlePayment("upi")} />
                                <PaymentMethodBtn name="PhonePe" onClick={PhonePe_handlePayment} />
                            </div>

                            <div className="mt-5 flex items-center justify-between border-t border-dashed border-orange-100 pt-4 text-xs text-gray-400">
                                <span>Powered by Razorpay &amp; PhonePe</span>
                                <div className="flex items-center gap-1">
                                    <FaShieldAlt className="text-green-500" size={12} />
                                    <span className="font-medium text-green-600">Secured</span>
                                </div>
                            </div>
                        </div>
                    </div>
...
export default function PlansDetails() {
    return (
        <Suspense
            fallback={
                <div className="flex min-h-screen items-center justify-center pt-16" style={{ backgroundColor: CREAM }}>
                    <div className="h-10 w-10 animate-spin rounded-full border-2 border-orange-200 border-t-[#FF5C00]" />
                </div>
            }
        >
            <PlansDetailsContent />
