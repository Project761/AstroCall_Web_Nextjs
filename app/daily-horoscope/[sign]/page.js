...
const HoroscopeDetails = () => {
  ...
  const horoscopename1 = sign;

  const router = useRouter();

  const [activeButton, setActiveButton] = useState("daily");
  const [horoscopedata, sethoroscopedata] = useState(null);
  const [horoscopename, sethoroscopename] = useState(horoscopename1 ? horoscopename1 : "Aries");
  const [Datehoroscope, setDatehoroscope] = useState(null);
  const [selectedTimeFrame, setSelectedTimeFrame] = useState("");
  const [activeDays, setActiveDays] = useState("current");
  const [activeCategory, setActiveCategory] = useState("Personal");

  const { LanguageDropdown, setLanguageDropdown, setisLogin, FAQData } = useContext(MenuContext);
  ...
  const categories = [
    { name: "Personal", key: "personal", icon: "🌸" },
    { name: "Health", key: "health", icon: "❤️" },
    { name: "Profession", key: "profession", icon: "📊" },
    { name: "Emotions", key: "emotions", icon: "😊" },
    { name: "Travel", key: "travel", icon: "✈️" },
    { name: "Luck", key: "luck", icon: "⭐" },
  ];
  ...
  //   navigate(`/daily-horoscope/details/${value}`);
  //   const { month, date, year } = currentDate;
  //   if (buttonName === "daily") {
  //     setSelectedTimeFrame(`${month} ${date}, ${year}`);
  //   }
  // };
  // const handleButtonClick = (buttonName, sign = horoscopename) => {
  //   setActiveButton(buttonName);
  //   navigate(`/daily-horoscope/details/${sign}`);
  // };
  ...
  useEffect(() => {
    if (horoscopename1) {
      ...
      sethoroscopename(horoscopename1);
    }
  }, [horoscopename1, activeButton, activeDays, LanguageDropdown]);
  ...
  const filteredDataFAQ = horoscopename1 ? FAQData?.filter(item => item?.HoroscopeName === horoscopename1) : '';
  ...
  const horoscopeSign = horoscopename1 || horoscopename || "Aries";
  const horoscopeTitle = `${horoscopeSign} ${activeButton.charAt(0).toUpperCase() + activeButton.slice(1)} Horoscope`;

  return (
    <>
      <SEO
        title={`${horoscopeTitle} - Daily Predictions & Guidance | AstroCall`}
        ...
        keywords={`${horoscopeSign} horoscope, ${horoscopeSign} ${activeButton} horoscope, ${horoscopeSign} predictions, ${horoscopeSign} astrology, zodiac sign ${horoscopeSign}`}
        canonical={`https://astrocall.live/daily-horoscope/${horoscopeSign}`}
        schema={{
            ...
            <div className="flex flex-col md:flex-row bg-white shadow-2xl rounded-xl sm:rounded-2xl md:rounded-3xl w-full max-w-6xl overflow-hidden">
              ...
              <div className="md:w-1/3 bg-gradient-to-b from-orange-500 to-orange-600 text-white p-4 sm:p-6 md:p-8 lg:p-10 space-y-4 sm:space-y-5 md:space-y-6">
                <div>
                  ...
                  >
                    {Horoscopeimages?.map((btn) => (
                      <option key={btn.name} value={btn.name}>{btn.name}</option>
                    ))}
                  </select>
                </div>
                ...
                <div className="space-y-2 sm:space-y-3">
                  ...
                  <div className="bg-white/20 rounded-lg sm:rounded-xl overflow-hidden">
                    ...
                    {activeButton === "daily" && buttonName2status && (
                      <div className="bg-white/10 px-2 sm:px-3 py-2 space-y-1 sm:space-y-2">
                        {buttonName2?.map((btn) => (
                          <button
                            key={btn.value}
                            onClick={() => {
                              setActiveDays(btn.value);
                              if (activeButton === "daily") handleButtonClick("daily");
                            }}
              ...
              <div className="md:w-2/3 p-4 sm:p-6 md:p-8">
                {horoscopedata ? (
                  categories.map((cat) => (
                    <div key={cat.key} className="mb-4 sm:mb-5 md:mb-6">
                      ...
                      {cat.key === "luck" ? (
                        <ul className="list-disc pl-5 sm:pl-6 text-gray-600 space-y-1 text-sm sm:text-base">
                          {horoscopedata.luck?.map((item, idx) => (
                            <li key={idx}>{item}</li>
                          ))}
                        </ul>
                      ) : (
                        <p className="text-gray-600 leading-relaxed text-sm sm:text-base">{horoscopedata[cat.key] || ""}</p>
                      )}
                    </div>
                  ))
                ) : (
