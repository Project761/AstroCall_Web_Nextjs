...
const GemstoneAddress = () => {
    ...
    const [Editval, setEditval] = useState([loginUserData]);
    ...
    // Get product data from sessionStorage (since Next.js doesn't have router state)
    useEffect(() => {
    ...
    useEffect(() => {
        if (UserLoginId) {
            Get_Data_UserAddress(UserLoginId);
        }
    }, [UserLoginId]);
    ...
    return (<>
                ...
                <div className="main-container px-3 sm:px-4">
                    <div className="flex flex-col items-center py-6 sm:py-8 md:py-10">
                        ...
                        <div className="flex flex-wrap gap-4 sm:gap-6 w-full max-h-[300px] sm:max-h-[400px] overflow-y-auto justify-center pr-2 sm:pr-3">
                            {addresses?.length > 0 ? (addresses.map((address) => (<div key={address?.ID} className="border border-orange-400 p-4 sm:p-5 rounded-lg sm:rounded-xl shadow-lg w-full sm:w-80 md:w-96 bg-white relative hover:shadow-xl transition duration-200" onClick={() => {
            ...
            }} className="w-full py-2 sm:py-2.5 bg-orange-500 hover:bg-orange-600 text-white rounded-lg font-semibold mt-2 text-sm sm:text-base transition shadow-md hover:shadow-lg">
