...
const LoadingIndicator = ({ color = "#F37254", size = "medium" }) => {
...
const GemstonePlanDetails = () => {
    const { loginUserData, setGemstonereviewstatus, Get_Data_RazorPayKey, RazorPayKey } = useMenuContext();
    const UserLoginId = typeof window !== 'undefined' ? localStorage.getItem("UserLoginId") || '' : '';
    const AddressLocationID = typeof window !== 'undefined' ? sessionStorage.getItem("AddressLocationID") || '' : '';
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            ...
            if (storedProduct) {
                try {
                    setGemstonedata(JSON.parse(storedProduct));
                    setLoading(false);
                }
                catch (error) {
    ...
    const Get_Data_gemstone = async () => {
        ...
        try {
            ...
            if (res) {
                const filteredData = res?.filter((item) => item?.GemstoneID == gemstonedata?.GemstoneID);
                if (filteredData?.length > 0) {
                    // Update gemstonedata with fresh data if needed
                }
            }
        }
        catch (error) {
    ...
    const Insert_GemstoneOrder_Data = async (OrderId) => {
        try {
            const val = {
                'OrderNumber': "OrderNumber",
                'GemstoneId': gemstonedata?.GemstoneID,
                'GemstoneName': gemstonedata?.HeadingDescription,
                'UserID': UserLoginId,
                'OrderStatus': "OrderStatus",
                'PaymentStatus': "PaymentStatus",
                'Amt': gemstonedata?.originalPrice,
                'GSTAmt': gemstonedata?.GSTAmt,
                'Qty': '1',
                'LocationID': AddressLocationID,
                'TransactionID': OrderId,
                'CreatedByUser': '1'
            };
            const res = await TokenWithDeleteUpadateAdd('GemstoneOrder/Insert_GemstoneOrder', val);
            if (res) {
                if (typeof window !== 'undefined') {
                    sessionStorage.setItem("GemstoneOrder", JSON.stringify(res));
                }
                router.push("/gemstone");
                if (setGemstonereviewstatus) {
    ...
    const handlePayment = async () => {
        try {
            ...
            if (response?.OrderId) {
                const options = {
                    key: RazorPayKey,
                    amount: gemstonedata?.TotalAmt * 100,
                    currency: "INR",
                    name: "AstroCall",
                    description: "Test Payment",
                    order_id: response.OrderId,
                    notes: {
                        GstAmt: gemstonedata?.GSTAmt ? gemstonedata?.GSTAmt : "0",
                        BonusAmt: "0",
                        UserId: UserLoginId,
                        PaymentType: "Gemstone",
                        ProductName: gemstonedata?.HeadingDescription,
                        "ProductId": gemstonedata?.GemstoneID
                    },
                    handler: function (paymentResponse) {
                        if (paymentResponse?.razorpay_order_id) {
                            Insert_GemstoneOrder_Data(paymentResponse?.razorpay_order_id);
                        }
                    },
                    prefill: {
    ...
    const PhonePe_handlePayment = async () => {
        try {
            ...
            if (res) {
                if (typeof window !== 'undefined') {
                    sessionStorage.setItem("MerchantIdGemstone", res?.MerchantOrderId);
                }
                await Insert_GemstoneOrder_Data(res?.MerchantOrderId);
        ...
        const renderIcon = () => {
            ...
            <g>
              <path fillRule="evenodd" clipRule="evenodd" d="M85.14,14.83L43.26,40.28h11.91l30.92-18.79l4.54-2.76l13.09,21.55h7.62 c1.66,0,3.16,0.68,4.25,1.76l0,0c1.09,1.09,1.77,2.59,1.77,4.24v70.59c0,1.65-0.68,3.15-1.76,4.23v0.01 c-1.09,1.09-2.59,1.76-4.25,1.76l-105.33,0c-1.66,0-3.16-0.67-4.25-1.76v-0.01C0.68,120.02,0,118.52,0,116.88V46.28 c0-1.65,0.68-3.16,1.76-4.24c1.09-1.09,2.59-1.76,4.25-1.76h2.5L73.53,0.77v0C74.36,0.27,75.3,0,76.27,0 c0.42,0,0.84,0.05,1.25,0.15c1.36,0.33,2.54,1.19,3.26,2.39v0l6.63,10.91L85.14,14.83z M5.89,45.62 c-0.23,0.25-0.42,0.53-0.56,0.84v8.69h106.68v-8.87c0-0.19-0.07-0.36-0.19-0.47h-0.01c-0.12-0.12-0.29-0.2-0.48-0.2H6.01 C5.97,45.61,5.93,45.61,5.89,45.62z M15.98,84.71h19.05v7.13H15.98V84.71z M15.98,101.59h53.25v6.43H15.98V101.59z M86.21,84.71h19.05v7.13H86.21V84.71z M62.8,84.71h19.05v7.13H62.8V84.71z M39.39,84.71h19.05v7.13H39.39V84.71z M112.01,75.14H5.33v41.73c0,0.19,0.07,0.36,0.2,0.48l0.01,0c0.13,0.13,0.3,0.2,0.47,0.2l105.33,0c0.18,0,0.35-0.08,0.48-0.2 l0,0c0.13-0.13,0.2-0.3,0.2-0.48V75.14z"/>
                ...
                return (<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 122.88 38.52" style={{ enableBackground: 'new 0 0 122.88 38.52' }} xmlSpace="preserve">
    ...
    if (loading) {
        ...
        <div className="flex justify-center items-center py-20">
          <LoadingIndicator color="#F37254" size="medium"/>
    ...
    if (!gemstonedata) {
        ...
        <div className="flex justify-center items-center py-20">
          <div className="text-center">
            ...
            <p className="text-gray-500 text-lg">No gemstone data found</p>
            <button onClick={() => router.push("/gemstone")} className="mt-4 px-6 py-2 bg-orange-500 text-white rounded-lg hover:bg-orange-600">
              Back to Gemstones
            </button>
          </div>
        </div>
    ...
    return (<>
      <SEO title="Gemstone Plan Details" description="Gemstone plan details." canonical="https://astrocall.live/checkout/payment" type="website" schema={{
            "@context": "https://schema.org",
            "@type": "Product",
            name: "Gemstone Plan Details",
            description: "Gemstone plan details.",
            url: "https://astrocall.live/checkout/payment"
        }}/>
      <Header />
      ...
      <div className="bg-[#F973160D] py-8 sm:py-12 md:py-16 lg:py-20 relative">
        ...
        <div>
          <div className="heading main-container px-3 sm:px-4 py-6 sm:py-8 md:py-10">
              ...
              <div className="lg:w-1/2 w-full bg-orange-50 p-4 sm:p-6 md:p-8 lg:p-10 rounded-xl sm:rounded-2xl shadow-md border">
                ...
                <div className="space-y-3 sm:space-y-4 text-xs sm:text-sm text-gray-700">
                  <div className="flex justify-between border-b pb-2">
                    ...
                    <span>₹ {gemstonedata?.CurrentPrice || 0}</span>
                  </div>
                  <div className="flex justify-between border-b pb-2">
                    <span className="font-medium">GST Amt:</span>
                    <span>₹ {gemstonedata?.GSTAmt || 0}</span>
                  </div>
                  <div className="flex justify-between border-b pb-2">
                    <span className="font-medium">Shipping Charges:</span>
                    <span>₹ 0</span>
                  </div>
                  <div className="flex justify-between font-semibold text-base sm:text-lg pt-2">
                    <span>Total Payable:</span>
                    <span className="text-orange-600">₹ {gemstonedata?.TotalAmt || 0}</span>
                  </div>
                </div>
              </div>

              <div className="lg:w-1/2 w-full bg-white p-4 sm:p-6 rounded-xl sm:rounded-2xl shadow-md border">
                <h2 className="text-lg sm:text-xl font-bold mb-4 sm:mb-6 text-primaryColor text-center">Choose Payment Method</h2>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 text-xs sm:text-sm">
                  <PaymentMethodBtn name="Card" onClick={() => handlePayment()}/>
                  <PaymentMethodBtn name="Wallets" onClick={() => handlePayment()}/>
                  <PaymentMethodBtn name="Paytm" onClick={() => handlePayment()}/>
                  <PaymentMethodBtn name="GPay" onClick={() => handlePayment()}/>
                  <PaymentMethodBtn name="BHIM UPI" onClick={() => handlePayment()}/>
...
export default GemstonePlanDetails;
